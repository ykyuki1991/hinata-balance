import {H,STAGES} from './engine.js?v=6.1.0';
...
export class Renderer{
 ...
 effect(e){const c=e.type==='return'?RED:e.type==='damage'?RED:e.type==='join'?'#bbcaf5':TEAL;if(['stamp','receipt','join','return','damage','hit','reapply','batch'].includes(e.type)){for(let i=0;i<(this.reduced?2:e.type==='hit'?2:12)&&this.particles.length<140;i++){const a=Math.random()*6.28;this.particles.push({x:e.x,y:e.y,vx:Math.cos(a)*(20+Math.random()*75),vy:Math.sin(a)*70,life:.25+Math.random()*.4,color:c});}if(e.type!=='hit')this.rings.push({x:e.x,y:e.y,life:.5,r:10,color:c});if(['return','reapply','join','batch'].includes(e.type))this.stamps.push({x:e.x,y:e.y,life:1.4,label:e.type==='return'?'差戻し':e.type==='reapply'?'再申請':e.type==='batch'?'一括 +'+e.bonus:'合流',color:c});if(e.type==='damage')this.shake=this.reduced?0:4;}if(e.type==='stageClear'){this.shake=this.reduced?0:2;}}
 ...
 for(const h of g.hazards){if(h.lane){c.globalAlpha=h.warn>0?.12:.32;this.rect(h.x-13,h.y,26,462-h.y,4,RED);c.globalAlpha=1;c.setLineDash(h.warn>0?[4,5]:[]);this.line([[h.x,h.y],[h.x,462]],h.warn>0?'#ecae89':RED,h.warn>0?1:3);c.setLineDash([]);this.arrow(h.x,455,Math.PI/2,RED,6);continue;}if(h.warn>0){c.globalAlpha=.4;const n=Math.hypot(h.vx,h.vy);this.line([[h.x,h.y],[h.x+h.vx/n*70,h.y+h.vy/n*70]],RED,1);this.circle(h.x,h.y,10*(1-h.warn/.55),null,RED,2);c.globalAlpha=1;}else{this.circle(h.x,h.y,7,'#553439',RED,2);this.circle(h.x,h.y,2,'#ffd6a1');}}
 if(g.choice){for(const [i,x]of [w*.25,w*.75].entries()){const active=i===0?g.x<w*.34:g.x>w*.66;this.rect(x-36,450,72,25,8,active?'#496c63':'#18343dcc',active?PAPER:'#8bafa3');this.text(i===0?'単線':'並行',x,467,12,PAPER);if(active)this.rect(x-30,478,60*Math.min(1,(g.choice.dwell||0)/.65),2,1,TEAL);if(i){this.line([[x-10,445],[x-10,438],[x+10,438],[x+10,445]],'#c1ccf5',2);}else this.line([[x,437],[x,445]],TEAL,2);}}
 this.ship(g,t);
